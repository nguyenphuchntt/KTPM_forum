...
func main() {
	...
	log.Printf("Starting Quarantine Watcher...")
	log.Printf("Watching container '%s' for new files...", quarantineContainer)
...
func processBlob(ctx context.Context, serviceURL azblob.ServiceURL, quarantineURL, productionURL azblob.ContainerURL, blobName string) {
	log.Printf("Processing %s...", blobName)
	...
	log.Printf("Validation PASSED for %s. Promoting...", blobName)
	...
	if err != nil {
		log.Printf("Failed to copy %s: %v", blobName, err)
		return
	}
	...
	if err != nil {
		log.Printf("Warning: Failed to delete %s from quarantine: %v", blobName, err)
	} else {
		log.Printf("Successfully promoted %s", blobName)
	}
}
