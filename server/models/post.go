...
func FetchCreatedPostsByUser(db *sql.DB, user_id int, currentPage int) ([]Post, int, error) {
	...
	LIMIT 10 OFFSET ? ;
	`
	rows, err := database.QueryWithMetrics(db, "select_posts_by_user", query, user_id, currentPage)
	if err != nil {
		log.Println("Error executing query:", err)
		return nil, 500, err
	}
...
func FetchLikedPostsByUser(db *sql.DB, user_id int, currentPage int) ([]Post, int, error) {
	...
	LIMIT 10 OFFSET ? ;
	`
	rows, err := database.QueryWithMetrics(db, "select_liked_posts", query, user_id, currentPage)
	if err != nil {
		log.Println("Error executing query:", err)
		return nil, 500, err
	}
...
func StorePost(db *sql.DB, user_id int, title, content string) (int64, error) {
	...
	result, err := database.ExecWithMetrics(db, "insert_post", query, user_id, title, content)
	if err != nil {
